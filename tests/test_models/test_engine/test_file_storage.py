...
class TestFileStorage(unittest.TestCase):
    ...
    @unittest.skipIf(models.storage_t == 'db', "not testing file storage")
    def test_get(self):
        """Test that get returns an object of a class based on id"""
        ...
        self.assertIsNot(obj, storage.get(State, obj.id + 'invsuf'))
        self.assertIsNone(storage.get(State, obj.id + 'invsuf'))
        self.assertIsNone(storage.get(State, 29))
        self.assertIsNone(storage.get(None, obj.id))
        self.assertIsNone(storage.get(int, obj.id))
        with self.assertRaises(TypeError):
            storage.get(State, obj.id, 'invsuf')
        with self.assertRaises(TypeError):
            storage.get(State)
        with self.assertRaises(TypeError):
            storage.get()
    ...
    @unittest.skipIf(models.storage_t == 'db', "not testing file storage")
    def test_count(self):
        """Test that count returns the number of objects based on class"""
        storage = models.storage
        ...
        self.assertEqual(storage.count(), storage.count(None))
        prev_cnt = storage.count(State)
        State(name='Georgia').save()
        self.assertGreater(storage.count(State), prev_cnt)
        Amenity(name='WiFi').save()
        self.assertGreater(storage.count(), storage.count(State))
        with self.assertRaises(TypeError):
            storage.count(State, 'invld')
